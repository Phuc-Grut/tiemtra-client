...
//         )}
//       </Box>
// {/* KO DÙNG TABLE DÙNG DATATABLE GIỐNG PRODUCTTABLE */}
...
const AttributeTable = () => {
  ...
  const invalidateAllAttributeData = () => {
    queryClient.invalidateQueries({
      predicate: (query) => query.queryKey[0] === "attributes",
    });
  };

  const handleConfirmDelete = async (attributeIds?: number[]) => {
    const ids: number[] = (attributeIds ?? selected).filter(
    ...
    try {
      const res = await attributeApi.deleteAttributeApi(ids);
      if (res.data.success) {
        showSuccess("Xoá thành công!");
  ...
  return (
    ...
    >
      ...
      >
        ...
        {selected.length > 0 && (
          <Button
            variant="contained"
            size="small"
            startIcon={<DeleteIcon sx={{ color: "white", fontSize: 16, marginRight: "-6px" }} />}
            sx={{
              marginLeft: "12px",
              textTransform: "none",
              fontSize: "13px",
      ...
      {/* Table */}
      <DataTableContainer<IAttribute>
        data={attributes}
        selected={selected}
        setSelected={setSelected}
        columns={[
          { key: "attributeId", label: "Mã thuộc tính", render: (r) => r.attributeId },
          { key: "name", label: "Tên thuộc tính", render: (r) => r.name },
          { key: "description", label: "Mô tả", render: (r) => r.description },
          { key: "creatorName", label: "Người tạo", render: (r) => r.creatorName },
          {
            key: "updatedAt",
            label: "Cập nhật",
            render: (r) => formatVietnamTime(r.updatedAt || r.createAt),
          },
        ]}
        isLoading={isLoading}
        error={!!error}
        ...
        toggleSort={(field) =>
          setFilter((prev) => ({
            ...prev,
            sortBy: prev.sortBy === field ? undefined : field,
          }))
        }
        getRowId={(r) => r.attributeId}
        onRowClick={(r) => {
      ...
      <Box mt={0} display="flex" justifyContent="center" alignItems="center" paddingBottom={0.5} gap={2}>
        <CustomPagination
          pageNumber={filter.pageNumber}
          totalPages={maxPages}
          setPageNumber={(newPage) => {
            setFilter((prev) => ({ ...prev, pageNumber: newPage }));
            navigate(`/admin/attribute/?page=${newPage}`);
          }}
        />
        <Box display="flex" alignItems="center" gap={1} maxHeight={25}>
          <TextField
      ...
      <ModalConfirm
        ...
        onConfirm={handleConfirmDelete}
        showConfirmButton
        message="Sau khi xóa, liên kết giữa thuộc tính và danh mục, sản phẩm sẽ bị mất, bạn có xác nhận xóa"
      />
    </Box>
  );
};
